...
namespace pvrtex {
  ...
  const float Wavelet::kBasicFilter[] = { 0.5f, 0.5f };
  ...
  Eigen::MatrixXi Wavelet::Upscale(Eigen::MatrixXi orig) {
    ...
    for (int j = 0; j < result.rows(); ++j) {
      for (int i = 0; i < result.cols(); ++i) {
        ...
        x = (i>>2);   /* (i/4) */
        y = (j>>2);   /* (j/4) */
        x1 = util::Clamp(x+1, 0, orig.cols()-1);
        y1 = util::Clamp(y+1, 0, orig.rows()-1);
        x_diff = (i - (x*4)) * ONE_FOURTH;
        y_diff = (j - (y*4)) * ONE_FOURTH;
        
        /* Get the colors of the neighboring pixels */
        a = util::MakeColorVector(orig(y, x));
        b = util::MakeColorVector(orig(y, x1));
        c = util::MakeColorVector(orig(y1, x));
        d = util::MakeColorVector(orig(y1, x1));
        
        /* Do the bilinear interpolation for each channel */
        for (int k = 0; k < 4; ++k) {
          color(k) = static_cast<int>(
                         util::lerp(util::lerp(static_cast<float>(a(k)),
                                               static_cast<float>(b(k)),
                                               x_diff),
                                    util::lerp(static_cast<float>(c(k)),
        ...
        /* Store the result */
        result(j, i) = util::MakeRGBA(color);
      }
    }
